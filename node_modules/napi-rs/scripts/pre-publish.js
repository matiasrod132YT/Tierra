...
class PrePublishCommand extends clipanion_1.Command {
    ...
    execute() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const { packageJsonPath, platforms, version, muslPlatforms, packageName, binaryName, } = consts_1.getNapiConfig(this.configFileName);
            ...
            for (const name of [...platforms, ...muslPlatforms]) {
                const pkgDir = path_1.join(process.cwd(), this.prefix, name);
                const filename = `${binaryName}.${name}.node`;
                debug(`Read [${chalk_1.default.greenBright(filename)}] content`);
                const bindingFile = yield utils_1.readFileAsync(path_1.join(process.cwd(), filename));
                const dstPath = path_1.join(pkgDir, filename);
                yield utils_1.writeFileAsync(dstPath, bindingFile);
                debug(`Write [${chalk_1.default.yellowBright(dstPath)}] content`);
                if (!this.isDryRun) {
                ...
                if (!this.isDryRun) {
                    ...
                    try {
                        ...
                        debug(`Param: ${{ owner, repo, tag: pkgInfo.tag, filename: dstPath }}`);
                        console.error(e);
                    }
                }
            }
